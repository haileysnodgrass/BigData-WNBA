...
def _parse_minutes(min_sec_series: pd.Series) -> pd.Series:
    """Convert 'MM:SS' strings to total minutes as a float."""
    def _convert(val):
        try:
            parts = str(val).split(':')
            ...
            return None
    return min_sec_series.apply(_convert)
...
def clean_game_logs(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    - Converts MIN_SEC to float minutes
    ...
    df['MINUTES']   = _parse_minutes(df['MIN_SEC'])
